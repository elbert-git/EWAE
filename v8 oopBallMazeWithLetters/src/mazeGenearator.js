// todo
// create generate maze with letters
// add isLetters function createMazeFromArray();


// ----- utility functions
function createMazeFromArray(mazeData){
    // maze generation variables
    const mazeDataArray = mazeData;
    let currentAddress = {xIndex:0, yIndex: 0};
    const mazeLength = mazeData.length;
...
function imprintLetter(mazeData, x, y, letterPattern){
    for (let yIndex = 0; yIndex < letterPattern.length; yIndex++) {
        const row = letterPattern[yIndex];
        for (let xIndex = 0; xIndex < row.length; xIndex++) {
            const cellToImprint = row[xIndex]
            if(cellToImprint){
                mazeData[xIndex][yIndex].isLetter = true;
            }
        }
    }
}
function imprintLines(mazeData, margin, lines){
    for (let lineIndex = 0; lineIndex < lines.length; lineIndex++) {
        const lineMargin = lines[lineIndex].margin;
        const word = lines[lineIndex].word
        const startX = 0;
        const startY = 0; 
        imprintLetter(mazeData, startX, startY, lines)
    }
}
...
export function GenerateMaze(width, height){
    ...
    const generatedMazeData = createMazeFromArray(mazeDataArray);
    return generatedMazeData;
}

export function GenerateMazeWithLetters(margin, lines){
    // main variables
    const letterWidth = 5;
    ...
    //find longest word
    let longestWord = lines[0];
    for (let index = 0; index < lines.length; index++) {
        if(lines[index].word.length > longestWord.word.length){
            longestWord = lines[index];
        }
    }

    // maze parameters
    const mazeWidth = longestWord.length*letterWidth + letterMargin*(longestWord.length-1) + margin + margin;
    const mazeHeight = letterHeight*lines.length + letterMargin*(lines.length-1) + margin + margin;

    //create plain maze
    const mazeData = []
    for (let xIndex = 0; xIndex < mazeWidth; xIndex++) {
        const column = [];
        for (let yIndex = 0; yIndex < mazeHeight; yIndex++) {
            const cell = createMazeData()
    ...
    //imprint letters
    imprintLines(mazeData, margin, lines)
     
    //generate maze loop
    const finalMaze = createMazeFromArray(mazeData)
    return finalMaze;
}